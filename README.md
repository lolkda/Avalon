...
1. **环境变量配置**：
   ...
   - **注意**：当环境变量与框架 `env` 配置冲突时，框架变量将优先覆盖系统环境变量。

### 多容器配置
2. **多容器支持**：
   - 必须配置以下 `openApi` 开头的变量以启用多容器功能：
     ```bash
     openApi_xxx_isOpen=""  # 启用容器（设置为 "true" 或 "1" 以启用）。
...
### 代理配置
3. **任务使用代理**：
   - 填写 `PROXY_URL` 到环境变量中然后根据api或代理池填写 `PROXY_Method` 类型1是api代理，2是代理池
   - **注意**：未配置 `PROXY_URL` 变量时网络IO任务执行将使用本机IP执行
